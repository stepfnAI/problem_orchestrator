...
class Step9ModelInferenceOrchestrator:
    ...
    def _run_inference(self):
        ...
        if not model_id or infer_df is None:
            self.view.show_message("❌ Required data not found. Please complete previous steps.", "error")
            return
        ...
        try:
            ...
            with self.view.display_spinner("Loading selected model..."):
                ...
                if not training_features:
                    raise ValueError("Training features information not found in model metadata")
            ...
            if self.view.display_button("🏁 Finish Pipeline"):
                try:
                    ...
                    if self.model_manager.cleanup():
                        self.view.show_message("✅ Models and registry cleaned up successfully", "success")
                    else:
                        self.view.show_message("⚠️ Error cleaning up models", "warning")
                        return
                    ...
                    self.view.show_message(f"❌ Error during pipeline completion: {str(e)}", "error")
