...
class DataSplitting:
    ...
    def execute(self):
        ...
        if not splitting_started:
            ...
            print(f"DEBUG: In data splitting - df shape: {df.shape if df is not None else 'None'}")
            print(f"DEBUG: In data splitting - df columns: {list(df.columns) if df is not None else 'None'}")
            print(f"DEBUG: In data splitting - mappings: {mappings}")
            print(f"DEBUG: In data splitting - target column: {mappings.get('target')}")
            ...
            print(f"DEBUG: In data splitting - target in df columns: {mappings.get('target') in df.columns if df is not None and mappings.get('target') else False}")
